drop policy if exists membros_upload_proprio on storage.objects;

create policy membros_public_registration_insert
on storage.objects
for insert
to anon, authenticated
with check (
  bucket_id = 'public-assets'
  and (storage.foldername(name))[1] = 'membros'
);
...
begin
  ...
  values (
    ...
    nullif(new.raw_user_meta_data ->> 'telefone_residencial', ''),
    nullif(new.raw_user_meta_data ->> 'telefone_comercial', ''),
  ...
  set
    ...
    telefone_residencial = coalesce(excluded.telefone_residencial, public.membros.telefone_residencial),
    telefone_comercial = coalesce(excluded.telefone_comercial, public.membros.telefone_comercial),
